...
//        plasma_tune_geqrf(plasma, PlasmaComplexDouble, m, n);
    ...
    plasma_ompss_zge2desc(pA, lda, A, &sequence, &request);
    plasma_ompss_zge2desc(pQ, ldq, Q, &sequence, &request);

    // synchronization
    #pragma oss taskwait

    // Call the tile async function.
    plasma_ompss_zungqr(A, T, Q, work, &sequence, &request);

    // synchronization
    #pragma oss taskwait

    // Translate Q back to LAPACK layout.
    plasma_ompss_zdesc2ge(Q, pQ, ldq, &sequence, &request);
 ...
 *
 * @sa plasma_zungqr
 * @sa plasma_ompss_cungqr
 * @sa plasma_ompss_dorgqr
 * @sa plasma_ompss_sorgqr
 * @sa plasma_ompss_zgeqrf
 *
 ******************************************************************************/
void plasma_ompss_zungqr(plasma_desc_t A, plasma_desc_t T, plasma_desc_t Q,
                         plasma_workspace_t work,
